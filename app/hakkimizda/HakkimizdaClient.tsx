...
// app/hakkimizda/HakkimizdaClient.tsx
// Modal burada yönetilir — kullanıcı sayfadan ayrılmadan form açılır.
        ...
        <div onClick={e => e.stopPropagation()} style={{ background: "white", borderRadius: 20, width: "100%", maxWidth: 600, maxHeight: "92vh", overflowY: "auto", boxShadow: "0 24px 72px rgba(0,0,0,0.25)", animation: "hbox 0.28s cubic-bezier(.22,.68,0,1.2)", pointerEvents: "all" }}>
          ...
          {submitted ? (
            <div style={{ padding: "44px 26px", textAlign: "center" }}>
              ...
              <p style={{ color: "#4b5563", lineHeight: 1.7, maxWidth: 380, margin: "0 auto 8px" }}>
                En kısa sürede <strong style={{ color: "#1a2e4a" }}>{form.phone || "belirttiğiniz numara"}</strong>&apos;dan sizi arayacağız.
              </p>
              <p style={{ color: "#6b7280", fontSize: "0.875rem", maxWidth: 380, margin: "0 auto 28px" }}>
                WhatsApp otomatik açılmadıysa{" "}
                <a href="https://wa.me/905473803801" target="_blank" rel="noopener noreferrer" style={{ color: "#1a2e4a", fontWeight: 600 }}>buraya tıklayın</a>.
              </p>
              <button onClick={onClose} style={{ background: "linear-gradient(135deg, #1a2e4a, #243d61)", color: "white", border: "none", padding: "12px 32px", borderRadius: 10, fontWeight: 700, fontSize: "1rem", cursor: "pointer", fontFamily: "inherit" }}>
                Kapat
              </button>
            </div>
          ) : (
              ...
              <div className="h-half" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                <div>
                  ...
                  <div style={{ position: "relative" }}>
                    ...
                    <input type="text" name="parentName" required placeholder="Adınız Soyadınız" value={form.parentName} onChange={handleChange} className="h-inp" style={inp} />
                  </div>
                </div>
                <div>
                  <label style={lbl}>Öğrenci Adı *</label>
                  <div style={{ position: "relative" }}>
                    <span style={ico}><IconUser /></span>
                    <input type="text" name="studentName" required placeholder="Öğrencinin Adı" value={form.studentName} onChange={handleChange} className="h-inp" style={inp} />
                  </div>
                </div>
              </div>
              <div className="h-half" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                <div>
                  ...
                  <div style={{ position: "relative" }}>
                    ...
                    <input type="tel" name="phone" required placeholder="0555 555 55 55" value={form.phone} onChange={handleChange} className="h-inp" style={inp} />
                  </div>
                </div>
                <div>
                  <label style={lbl}>E-posta <span style={{ fontWeight: 400, color: "#9ca3af" }}>(isteğe bağlı)</span></label>
                  <div style={{ position: "relative" }}>
                    <span style={ico}><IconMail /></span>
                    <input type="email" name="email" placeholder="ornek@email.com" value={form.email} onChange={handleChange} className="h-inp" style={inp} />
                  </div>
                </div>
              </div>
    ...
    <article style={{ background: "white", borderRadius: 22, border: `2px solid ${sinirRenk}`, overflow: "hidden", boxShadow: "0 4px 24px rgba(0,0,0,0.07)" }}>
      <div style={{ background: `linear-gradient(135deg, ${renk}, ${renk}dd)`, padding: "32px 32px 28px", display: "flex", alignItems: "center", gap: 20 }}>
        <div style={{ width: 92, height: 92, borderRadius: "50%", border: "3px solid rgba(255,255,255,0.5)", flexShrink: 0, overflow: "hidden", background: "rgba(255,255,255,0.1)" }}>
          <Image src={foto} alt={`${isim} - Psikolojik Danışman`} width={92} height={92} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "top" }} />
        </div>
        <div>
      ...
      <div style={{ padding: "28px 32px" }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {odak.map((alan) => (
              <span key={alan} style={{ background: acikRenk, border: `1px solid ${sinirRenk}`, borderRadius: 8, padding: "5px 12px", fontSize: "0.78rem", fontWeight: 600, color: renk }}>{alan}</span>
            ))}
          </div>
        </div>
      </div>
